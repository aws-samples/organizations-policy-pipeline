...
def main():
    ...
    for statement in data:
        ...
        if statement["Guardrails"] != []:
            ...
            logger.info(f"Validating individual SCP policy '{statement['Policy']}' with Access Analyzer")
            ...
            if findings:
                logger.critical(
                    f"[!] Security findings were found in SCP policy {statement['Policy']}: {json.dumps(findings, indent=4)}"
                )
                sys.exit(1)
            else:
