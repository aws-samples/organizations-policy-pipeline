...
def mergeguardrails(guardrails_list, guardrails_folder):
    ...
    response1 = access_analyzer_client.check_no_new_access(newPolicyDocument=json.dumps(temp_optimized_policy), existingPolicyDocument=json.dumps(temp_original_policy), policyType='IDENTITY_POLICY')
    response2 = access_analyzer_client.check_no_new_access(newPolicyDocument=json.dumps(temp_original_policy), existingPolicyDocument=json.dumps(temp_optimized_policy), policyType='IDENTITY_POLICY')
    ...
    for page in paginator.paginate(locale='EN', policyDocument=json.dumps(optimized_policy), policyType='SERVICE_CONTROL_POLICY'):
        findings.extend(page['findings'])
    
    if not findings:
        logger.info("No findings found")
    else:
        logger.critical(f"Security findings were found in SCP policy: {json.dumps(findings, indent=4)}")
        sys.exit(1)
