...
def mergeguardrails(guardrails_list, guardrails_folder):
    ...
    policy = OrderedDict([("Version", "2012-10-17"), ("Statement", concatenate_policy_files(guardrails_list, guardrails_folder))])
    logger.debug(f"Value for policy: {policy}")
    
    logger.info(f"Length of the concatenated policy BEFORE optimization: {len(str(policy))}")
    ...
    response1 = access_analyzer_client.check_no_new_access(newPolicyDocument=json.dumps(temp_optimized_policy), existingPolicyDocument=json.dumps(temp_original_policy), policyType='RESOURCE_POLICY')
    response2 = access_analyzer_client.check_no_new_access(newPolicyDocument=json.dumps(temp_original_policy), existingPolicyDocument=json.dumps(temp_optimized_policy), policyType='RESOURCE_POLICY')
    ...
    for page in paginator.paginate(locale='EN', policyDocument=json.dumps(optimized_policy), policyType='RESOURCE_CONTROL_POLICY'):
        findings.extend(page['findings'])
    
    if not findings:
        logger.info("No findings found")
    else:
        logger.critical(f"Security findings were found in RCP policy: {json.dumps(findings, indent=4)}")
        sys.exit(1)
    ...
    logger.info(f"Length of the concatenated policy AFTER optimization: {len(str(optimized_policy_no_sid))}")
...
def concatenate_policy_files(guardrails_list, guardrails_folder):
    ...
    logger.info(f"The following guardrails will be merged in a single policy: {guardrails_list}")
    file_contents = []
    for file_name in guardrails_list:
        ...
        try:
            with open(file_full_path, 'r') as file:
                ...
                file_contents.extend([{k: v for k, v in statement.items() if k.lower() != 'sid'} for statement in content])
...
def remove_sids_from_policy(policy):
    ...
    new_policy['Statement'] = [{k: v for k, v in statement.items() if k.lower() != 'sid'} for statement in policy.get('Statement', [])]
...
def optimize_iam_policy(policy):
    ...
    for statement in statements:
        action_type = next((at for at in ('Action', 'NotAction') if at in statement), None)
        if action_type:
    ...
    for (resource, condition, effect, action_type), group in grouped_statements.items():
        if len(group) == 1:
            optimized_statements.append(group[0])
        else:
            merged_statement = OrderedDict([
                ('Effect', effect),
                (action_type, sorted(set(action for stmt in group for action in (stmt.get(action_type, []) if isinstance(stmt.get(action_type), list) else [stmt.get(action_type)])))),
                ('Resource', json.loads(resource)),
            ])
            if condition != '{}':
                merged_statement['Condition'] = json.loads(condition, object_pairs_hook=OrderedDict)
            optimized_statements.append(merged_statement)
    
    optimized_statements.extend([statement for statement in statements if 'Action' not in statement and 'NotAction' not in statement])
